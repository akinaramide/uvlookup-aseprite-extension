function init(plugin)
	...
	if plugin.preferences.uvLookups == nil then
		plugin.preferences.uvLookups = {}
	end

	local uvLookups = plugin.preferences.uvLookups

	local function getLookupNames()
		local names = {}
		for name, _ in pairs(uvLookups) do
	...
	local function uvlookup()
		...
		function sync()
			...
			-- DEBUGGING LINE
			print("Cels in 'color' layer:", #lookupTopLayer.cels)
			if #lookupTopLayer.cels == 0 then
				app.alert("No cels found in 'color' layer")
				return
			end
			--
		...
		dialog:combobox {
			...
			label = "Saved Lookups",
			option = "default",
			options = getLookupNames(),
			onchange = function()
				local name = dialog.data.savedLookups
				if uvLookups[name] then
					uvLookup = uvLookups[name].uvLookup
					colorLookup = uvLookups[name].colorLookup
				end
			end
		}
		...
		dialog:button{
			text = "Save Lookup",
			onclick = function()
				local name = dialog.data.lookupName
				if name and #name > 0 then
					uvLookups[name] = {
						uvLookup = uvLookup,
						colorLookup = colorLookup
					}
					plugin.preferences.uvLookups = uvLookups
					app.alert("Lookup saved as: " .. name)
				end
			end
		}
